...
class Guild {
     ...
     *     .catch(console.error);
     */
    fetchMembers(query, limit, channel) { if(query===undefined)query = '';if(limit===undefined) limit = 0;
        return new Promise((resolve, reject) => {
			...
			if(this.client.user.bot) {
				...
				const chobj = {};
				const txtch = this.channels.filter(ch => ch.type == 'text');
				if(!txtch.size) return reject(new Error('No text channel to target for the OP14 gateway.'));
				
				chobj[channel.id || channel || this.systemChannelID || txtch.first().id] = [ [0, limit || 99] ];
