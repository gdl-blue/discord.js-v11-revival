...
class GuildChannel extends Channel {
    ...
    fetchMembers(limit) {
		if(this.type != 'text' && this.type != 'news') return false;
		
		if(this.client.user.bot) return Promise.resolve((ch => ch.guild.members.filter(member => member.permissionsIn(ch).has('VIEW_CHANNEL')))(this))
		else {
			return this.guild.fetchMembers('쥐', limit || 99, this.id);
		}
	}
